...
c     conditions.
c     
c     All of this takes place within a metric space.
c     
c     
      include 'basics.inc'

      call set_metric         ! set current working metric space

      do while (imetric.gt.0)
         call mesh_metric      
      enddo
...
c     
      ...
      item(3)       =             'GLOBAL REFINE'
      item(4)       =             'DELETE ELEMENT'
      item(5)       =             'ZOOM'
      item(6)       =             'SET GRID'
      item(7)       =             'REDRAW MESH'
      nchoic = 1
      ...
      if (choice.eq.'END    ELEMENTS') return
      if (choice.eq.'ADD    ELEMENT' ) call add_met_elem

      return
      end
c-----------------------------------------------------------------------
      subroutine add_met_elem  ! add element on current metric grid
      ...
      do ic=1,4
         ...
         if (ic.gt.1) then
            ...
            call drawc (xmouse,ymouse)
         endif

         x(nel,ic) = rmet(imet,iobj)  ! imet,iobj from menu call
         y(nel,ic) = smet(imet,iobj)
         z(nel,ic) = 0
      ...
      call draw_met_el(nel,'u',7)
      
      return
      end
c-----------------------------------------------------------------------
      subroutine draw_met(e,form,nr)  ! draw element on current metric grid
