...
static void gsi_via_known_map(gs_id *gs);
static void gsi_via_int_list(gs_id *gs);
static void get_ngh_buf(gs_id *gs);
static void get_ngh_list(gs_id *gs);
...
gs_init(register int *elms, int nel, int level)
{
  ...
  if (is_map_and_vertex_set())
    {
      gsi_via_known_map(gs);
      gsi_via_bit_mask(gs);
    }
  else
    {gsi_via_bit_mask(gs);}
  ...
  check_new_ngh_list(gs->num_nghs,gs->nghs);
...
#ifdef DEBUG  
  error_msg_warning("gsi_via_known_map() begin w/%d :: %d\n",my_id,num_nodes);
#endif
...
get_ngh_list(gs_id *gs)
{
  ...
  gs->num_nghs = num_nghs = get_proc_nghs(&nghs);
  gs->nghs = nghs;
...
get_ngh_buf(gs_id *gs)
{
  ...
  gs->nghs = (int *) bss_malloc(gs->num_nghs * INT_LEN);
...
#ifdef DEBUG
  error_msg_warning("start gs_gop_local_out()\n");
#endif
...
gs_gop_binary(gs_ADT gs, REAL *vals, rbfp fct)
{
  ...
  error_msg_warning("start gs_gop_binary()\n");
#endif
...
#ifdef DEBUG
  error_msg_warning("start gs_gop_local_binary()\n");
#endif
...
#ifdef DEBUG
  error_msg_warning("start gs_gop_local_in_binary()\n");
#endif
...
#ifdef DEBUG
  error_msg_warning("start gs_gop_pairwise_binary()\n");
#endif
...
{
#ifdef DEBUG
  error_msg_warning("start gs_gop_exists()\n");
#endif
...
#ifdef DEBUG
  error_msg_warning("start gs_gop_local_exists()\n");
#endif
...
#ifdef DEBUG
  error_msg_warning("start gs_gop_local_in_exists()\n");
#endif
...
#ifdef DEBUG
  error_msg_warning("start gs_gop_pairwise_exists()\n");
#endif
...
{
#ifdef DEBUG
  error_msg_warning("start gs_gop_max_abs()\n");
#endif
...
#ifdef DEBUG
  error_msg_warning("start gs_gop_local_max_abs()\n");
#endif
...
#ifdef DEBUG
  error_msg_warning("start gs_gop_local_in_max_abs()\n");
#endif
...
#ifdef DEBUG
  error_msg_warning("start gs_gop_pairwise_max_abs()\n");
#endif
...
{
#ifdef DEBUG
  error_msg_warning("start gs_gop_max()\n");
#endif
...
#ifdef DEBUG
  error_msg_warning("start gs_gop_local_max()\n");
#endif
...
#ifdef DEBUG
  error_msg_warning("start gs_gop_local_in_max()\n");
#endif
...
#ifdef DEBUG
  error_msg_warning("start gs_gop_pairwise_max()\n");
#endif
...
{
#ifdef DEBUG
  error_msg_warning("start gs_gop_min_abs()\n");
#endif
...
#ifdef DEBUG
  error_msg_warning("start gs_gop_local_min_abs()\n");
#endif
...
#ifdef DEBUG
  error_msg_warning("start gs_gop_local_in_min_abs()\n");
#endif
...
#ifdef DEBUG
  error_msg_warning("start gs_gop_pairwise_min_abs()\n");
#endif
...
{
#ifdef DEBUG
  error_msg_warning("start gs_gop_min()\n");
#endif
...
#ifdef DEBUG
  error_msg_warning("end gs_gop_min()\n");
#endif
}
...
#ifdef DEBUG
  error_msg_warning("start gs_gop_local_min()\n");
#endif
...
#ifdef DEBUG
  error_msg_warning("start gs_gop_local_in_min()\n");
#endif
...
#ifdef DEBUG
  error_msg_warning("start gs_gop_pairwise_min()\n");
#endif
...
#ifdef DEBUG  
  error_msg_warning("start gs_gop_local_times()\n");
#endif
...
#ifdef DEBUG  
  error_msg_warning("start gs_gop_local_in_times()\n");
#endif
...
#ifdef DEBUG  
  error_msg_warning("start gs_gop_pairwise_times()\n");
#endif
...
gs_print_template(register gs_id* gs, int who)
{
  ...
  if ((my_id == who) && (num_gs_ids))
      ...
      if (iptr = gs->pair_list)
	{
      ...
      printf("loc_node_pairs=%d\n",   gs->loc_node_pairs);
      printf("max_node_pairs=%d\n",   gs->max_node_pairs);
      printf("min_node_pairs=%d\n",   gs->min_node_pairs);
...
static
void
gs_print_stemplate(register gs_id* gs, int who)
{
  register int j, k, *iptr, *iptr2;
...
#ifdef DEBUG  
  error_msg_warning("start gs_gop_free()\n");
...
#ifdef DEBUG  
  error_msg_warning("start gs_dump_ngh()\n");
#endif
			 ...
			 register int step)
{
  ...
  do 
    {
      /* Should MPI_ANY_SOURCE be replaced by *list ? In that case do the
	 second one *list and do list++ afterwards */
      MPI_Irecv(in1, *size*step, REAL_TYPE, MPI_ANY_SOURCE, MSGTAG1 + *list++, 
		gs->gs_comm, msg_ids_in++); 
      in1 += *size++ * step;
    }
  while (*++msg_nodes);
  ...
  while (iptr = *msg_nodes++)
      ...
      while (*iptr >= 0)
	{
      ...
      MPI_Isend(dptr3, *msg_size++*step, REAL_TYPE, *msg_list++,
		MSGTAG1+my_id, gs->gs_comm, msg_ids_out++);
    }
