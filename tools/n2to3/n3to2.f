...
c
      ...
      read(10,80) string
      write(11,'(2x,a18)') ' 2 DIMENSIONAL RUN'
c
...
C
      ...
      IF (NCURVE.GT.0) THEN
         DO 50 ICURVE=1,NCURVE
            IF (nel.LT.1000) THEN
               READ(10,60,ERR=501,END=502) IEDG,IEG,R1,R2,R3,R4,R5,ANS
            ELSE
               READ(10,61,ERR=503,END=504) IEDG,IEG,R1,R2,R3,R4,R5,ANS
            ENDIF
            CURVE (1,IEDG,IEG)=R1
            CURVE (2,IEDG,IEG)=R2
            CURVE (3,IEDG,IEG)=R3
...
C
  ...
  504 WRITE(6,604)
      CALL EXIT
  601 FORMAT(2X,'ERROR 501 READING CURVE SIDE DATA'
     $    ,/,2X,'ABORTING IN ROUTINE RDCURVE.')
  602 FORMAT(2X,'ERROR 502 READING CURVE SIDE DATA'
     $    ,/,2X,'ABORTING IN ROUTINE RDCURVE.')
  603 FORMAT(2X,'ERROR 503 READING CURVE SIDE DATA'
     $    ,/,2X,'ABORTING IN ROUTINE RDCURVE.')
  604 FORMAT(2X,'ERROR 504 READING CURVE SIDE DATA'
     $    ,/,2X,'ABORTING IN ROUTINE RDCURVE.')
      RETURN
C
      END
