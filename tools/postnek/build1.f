...
c-----------------------------------------------------------------------
c
c         Write new coordinates to screen....   4/25/97  pff
c
          write (s,401) iel,icorn,xs,ys
  401     format('X-Y Coordinate:',i6,i4,1p2e13.4,'$')
          call  prs(s)
c
          write (s,402) 
  402     format('  $')
...
C               cyclic permutation (counter clock-wise):  reverse
  ...
  400              CONTINUE
     ...
     $              C3.LE.0.0.OR.C4.LE.0.0 ) THEN
                CALL PRS('ERROR in entering element.  Re-enter.$')
                CALL PRS('ERROR in entering element.  Re-enter.$')
                CALL PRS('ERROR in entering element.  Re-enter.$')
                IERR=1
                RETURN
...
C
   ...
   10 FORMAT(' Deleting elemenet',I5,' of',I5,'.$')
      CALL PRS(S)
   20 CONTINUE
C
      CALL COPYEL(NEL,IDEL)
...
c
      CALL PRS
     $('Push yellow button close to the corner you want changed.$')
      IFTMP =IFGRID
      IFGRID=.FALSE.
