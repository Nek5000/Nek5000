	  ...
	  if (!(my_id & mask))
	    {
	      ...
	      if (my_id > dest)
		{
		  MPI_Isend(vals,len,REAL_TYPE,dest,type,MPI_COMM_WORLD,
			    &msg_id);
		  MPI_Wait(&msg_id,&status);
		  /*msg_id = isend(type,vals,len,dest,0);
		    msgwait(msg_id);*/
		...
		{
		  type =  type - my_id + source;
		  MPI_Irecv(work,len,REAL_TYPE,MPI_ANY_SOURCE, 
			    type,MPI_COMM_WORLD,&msg_id);
		  MPI_Wait(&msg_id,&status);
		  /*msg_id = irecv(type,work,len);
		    msgwait(msg_id); */
...
#ifdef DEBUG
      if (!my_id) {printf("msg_size down = %d\n",size-stage_n);}
      fflush(NULL);
#endif
  ...
  for (edge = 0; edge < i_log2_num_nodes; edge++)
      ...
      if (!(my_id & mask))
	{
	  ...
	  if (my_id > dest)
	    ...
	    {
	      type =  type - my_id + source;
	      MPI_Irecv(dptr,len,REAL_TYPE,MPI_ANY_SOURCE, 
			type,MPI_COMM_WORLD,&msg_id);
	      MPI_Wait(&msg_id,&status);
	      /* msg_id = irecv(type, dptr,len);
		 msgwait(msg_id);*/
