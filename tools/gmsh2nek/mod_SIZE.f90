...
!
      ...
      character(32)  mshname_solid     ! MXSTLN=32 max string length in an exodus file

 
      integer totalNode,totalLine,totalQuad,totalHex,totalElem,bcNumber
      integer num_dim,num_elem
      integer aorb, aorb_solid ! file type in fmsh file header, 0 for ascii, 1 for binary
      integer ifsolid
      integer readfluid,preread
 
      integer eftot ! fluid element number
      integer startQuad,startHex
