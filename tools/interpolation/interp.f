C=======================================================================
      ...
      subroutine interpl_gen(ust,xst,u)
...
c     if(cat.eq.1 .or. (cat.eq.2 .and. dist.gt.1.e-20)) then
         print *, 'interpolation failed on:'
         print *, 'xst=', (xst(id),id=1,ndim)
         print *, 'cat,dist', cat, dist
         print *, 'ie,r,s,t=', ie, (t(id),id=1,ndim)
         call exitt ! sorry, didn't exactly find it
                    ! although it could be a boundary point (cat=2)
      endif
      call intp_evalpt(u(1,1,1,ie),Jm,ust)
...
c      print *, 'x', (x(id),id=1,ndim)
      do d=ndim,1,-1
         ...
         if(i.lt.0 .or. i.ge.bnd_hn) 
     $      write(6,*) 'cat',i,d,ii,(x(kk),kk=1,3),bnd_hn
         if(i.lt.0 .or. i.ge.bnd_hn) return
         ii=bnd_hn*ii+i
      enddo
      j=bnd_hash(ii+1)
      k=bnd_hash(ii+2)-j
      if(k.le.0) write(6,*) 'cat',j,k,ii,(x(kk),kk=1,3)
      if(k.le.0) return
...
c           call exitt
         ...
         nxyz = nx1*ny1*nz1
         ioff = nxyz*(ie-1) + 1
         call intp_evalpt(xm1(ioff),tJm,tx(1))
         call intp_evalpt(ym1(ioff),tJm,tx(2))
         if(if3d) call intp_evalpt(zm1(ioff),tJm,tx(3))
...
c----------------------------------------------------------
     ...
     $               ,ot(ldim),jact(ldim,ldim),cx(ldim)
      ...
      do while (dist.gt.tol .and. step.lt.maxstep)
         ...
         ioff = nxyz*(ie-1) + 1
         call intp_fulleval(
     $         xm1(ioff),Jm,Dm,cx(1),jact(1,1))
         call intp_fulleval(
     $         ym1(ioff),Jm,Dm,cx(2),jact(1,2))
         if(if3d) call intp_fulleval(
     $         zm1(ioff),Jm,Dm,cx(3),jact(1,3))
