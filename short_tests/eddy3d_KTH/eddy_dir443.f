...
c
     ...
     $              ,pd(lx2,ly2,lz2,lelt)
      ...
      call ortho  (pe)
      if (istep.eq.0) call outpost(ue,ve,vz,pe,t,'   ')

      call sub3   (ud,ue,vx,n)
      call sub3   (vd,ve,vy,n)
      call sub3   (pd,pe,pr,n2)
      if (istep.eq.nsteps) call outpost(ud,vd,vz,pd,t,'err')
...
c     NOTE ::: This subroutine MAY NOT be called by every process
      ...
      u0   = param(96)
      v0   = param(97)
      call exact(ux,uy,x,y,1,time,visc,u0,v0)
      uz=0.0
...
c-----------------------------------------------------------------------
      ...
      z0 = 0
      z1 = 0.2
      call rescale_x(xm1,x0,x1)  ! [0,2pi]
      call rescale_x(ym1,x0,x1)  ! [0,2pi]
      call rescale_x(zm1,z0,z1)  ! [0,0.2]
