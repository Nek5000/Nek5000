...
c-----------------------------------------------------------------------
      subroutine outsurf(v1,sv1,v2,nx)
c
      include 'SIZE'
      include 'TOTAL'
c
      real v1(lx1,ly1,lz1,1),v2(lx1,ly1,lz1,1)
c
      parameter (lt = lx1*ly1*lz1*lelt)
      common /scrns/ xyz(3,lt),v12(2,lt)   ! input is real*8
      real           xyz      ,v12

      parameter (lx = lx1*ly1*lz1)
      common /ctmp1/ xx(lx),yy(lx),zz(lx)
      common /ctmp0/ vv(lx),ww(lx)
c
      common /scruz/ tri(6*lt)
      integer tri
c
      common /scrmg/ txyz(6*lt)            ! output is real*4
      real*4         txyz

      common /scrsf/ t2  (4*lt)            ! output is real*4
      real*4         t2
...
c
         ...
         do e = 1,nelv
            ...
            do i=1,nx1-1
               ncell = ncell+1
               ioff=i-1+nx1*(j-1)+nx1*ny1*(k-1)+nx1*ny1*nz1*(e-1)
               l  = 0
               do kk=0,1
               do jj=0,1
               do ii=0,1
                  l = l+1
                  cell(l,ncell) = ioff + ii + jj*nx1 + kk*nx1*ny1
               enddo
               enddo
               enddo
            enddo
         ...
         dx = 2
         dx = dx/(nx1-1)   ! Build interpolation matrix to regular mesh
         do i=1,nx1
            xreg(i) = -1 + dx*(i-1) ! regular mesh
         enddo
         call igllm(jreg,jregt,zgm1,xreg,nx1,nx1,nx1,nx1)
      endif
c
      nxyz = nx1*ny1*nz1
      l = 0
      do e=1,nelv
         call hsmg_tnsr(xx,nx1,xm1(1,1,1,e),nx1,jreg,jregt)
         call hsmg_tnsr(yy,nx1,ym1(1,1,1,e),nx1,jreg,jregt)
         call hsmg_tnsr(zz,nx1,zm1(1,1,1,e),nx1,jreg,jregt)
         call hsmg_tnsr(vv,nx1,v1 (1,1,1,e),nx1,jreg,jregt)
         call hsmg_tnsr(ww,nx1,v2 (1,1,1,e),nx1,jreg,jregt)
         do i=1,nxyz
            l = l+1
            xyz(1,l) = xx(i)
            xyz(2,l) = yy(i)
...
c     call memestimate (cell,xyz,glo_num,v12,ncell,n,natt,iatt,sv1,
c    $                  tri,txyz,t2,ntri,ntnodes)
c
      ntri    = 0
      ntnodes = 0
      call isosurface (cell,xyz,glo_num,v12,ncell,n,natt,iatt,sv1
     $                ,tri,txyz,t2,ntri,ntnodes)

      write(6,*) nid,'ntri:',ntri,ntnodes,sv1
      call output_tri (txyz,ntnodes,lt,tri,ntri,lt  ,t2)
      if (ntnodes.gt.lt.or.ntri.gt.lt) call exitt

      return
      end
c-----------------------------------------------------------------------
      subroutine output_mat_r4 (x,m,n,l,ifhdr)
      ...
      if (ifhdr) ng = iglsum(n,1)
      mn = m*n
      ml = m*l

      if (nid.eq.0) then
         write(6,1) istep,time,ng
    1    format(i9,1pe14.7,i10,' triangles')
         if (ifhdr) call byte_write(ng,1)    ! write 4-byte words
         call byte_write(x ,mn)

c        if (ifhdr)  write(59,*) ng
c        if (m.eq.2) write(59,2) (x(k,1),k=1,mn)
c        if (m.eq.3) write(59,3) (x(k,1),k=1,mn)
   2     format(1p2e14.6)
   3     format(1p3e14.6)
      ...
      if (nid.eq.0) then
         do jnid=1,np-1
            ...
            len = 4
            ibf = 0
            call csend(mtype,ibf,len,jnid,0)
            lem = 4*ml
            call crecv3(mtype,x,len,lem)
            mn = len/4
            if (mn.gt.0) call byte_write(x,mn)
c           if (mn.gt.0.and.m.eq.2) write(59,2) (x(k,1),k=1,mn)
c           if (mn.gt.0.and.m.eq.3) write(59,3) (x(k,1),k=1,mn)
         enddo
      else
         mtype = nid
         len   = 4
         call crecv(mtype,ibf,len)
         len   = 4*mn
         call csend(mtype,x,len,0,0)
      endif
 
      return
      end
c-----------------------------------------------------------------------
      subroutine output_mat_i4 (x,m,n,l,ifhdr)
...
c-----------------------------------------------------------------------
      subroutine output_tri (txyz,nxyz,lxyz,tri,ntri,ltri,t2)
...
c     if (nid.eq.0) open(unit=59,file='surf.a',status='unknown')
      ...
      call izero(nnodes,np)    ! COMPUTE OFFSET in global address space
      nnodes(nid) = nxyz
      call igop(nnodes,nwork,'+  ',np)
      do i = 1,np-1
         nnodes(i) = nnodes(i) + nnodes(i-1) ! running sum
      enddo
      ...
      if (icalld.lt.4)
     $ write(6,12) nid,istep,icalld,nxyz,ntri,mtr0,mtri,nnodes(nid)
  12  format(i5,7i9,' mtri')

      call output_mat_r4 (txyz,3,nxyz,lxyz,.true.)
      call output_mat_i4 (tri ,3,ntri,ltri,.true.)
      call output_mat_r4 (t2  ,2,nxyz,lxyz,.false.)

c     if (nid.eq.0) close(59)
