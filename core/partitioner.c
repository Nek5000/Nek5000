...
int __attribute__((weak)) parMETIS_partMesh(int *part, long long *vl, int nel,
                                            int nv, int *opt, MPI_Comm ce) {
  int rank;
  MPI_Comm_rank(ce, &rank);

  if (rank > 0) goto barrier_before_abort;
  fprintf(stderr, "Error: parMETIS is not enabled during the build. Use "
                  "\"PARMETIS\" in PPLIST to enable it.");
  fflush(stderr);

barrier_before_abort:
  MPI_Barrier(ce);
  MPI_Abort(ce, EXIT_FAILURE);
}
