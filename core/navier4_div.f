...
c
      common /xyzu/ uic(lx1,ly1,lz1,lelv) , vic(lx1,ly1,lz1,lelv)
     $            , wic(lx1,ly1,lz1,lelv) , sic(lx1,ly1,lz1,lelv)
c
      logical ifprjp
C
...
c
      if (ifanls) then
         write(6,*) 'vtrans:',vtrans(1,1,1,1,1)
         call opcolv3(dv1,dv2,dv3,vx,vy,vz,vtrans)
         call opdiv  (respr,dv1,dv2,dv3)
      elseif (param(61).ne.0) then           ! Residual injection
         call opsub3  (dv1,dv2,dv3,vx,vy,vz,uic,vic,wic)
         CALL OPDIV   (RESPR,dv1,dv2,dv3)
      else
...
c
      call fill_div(dv1)                   ! Impose nontrivial divergence
      call col2    (dv1,bm2,ntot2)         
      call add2    (respr,dv1,ntot2)         
c
c     CALL LMACHRP (RESPR)
      CALL ORTHO   (RESPR)
C******************************************************************
...
c
      ...
      CALL SSNORMP (DV1,DV2,DV3)
C
      CALL EXTRAPP (PREXTR)
      CALL LAGPRES
      CALL ADD3  (PR,PREXTR,RESPR,NTOT2)
C
      RETURN
      END
c-----------------------------------------------------------------------
...
C
      ...
      COMMON /ORTHOV/ RHS(LTOT2,MXPREV),Pbar(LTOT2)
     $               ,Pnew(LTOT2)
...
c
   ...
   10 format(I6,1p3e12.4,i6,' alph1h ',a4)
c
