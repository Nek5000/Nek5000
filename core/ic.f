...
! CMT only
      ...
      COMMON /otherpvar/ PHIG(LX1,LY1,LZ1,LELV)
...
C     Initialize all fields:
   ...
   10 continue
      if(IFCMT)then
         call rzero(phig,ntott)
         call rzero(csound,ntott)
         call rzero(vtrans,ntott*3)
         call rzero(vdiff,ntott*3)
         call rzero(u,ntotcv)
      endif
...
! CMT-nek
         IF(IFCMT)THEN
            DO IEL=1,NEL
               ...
               DO I=1,NX1           
                  CALL NEKASGN (I,J,K,IEL)
                  CALL USERIC  (I,J,K,IEL)
                  IF (IFIELD.EQ.1) THEN
                     VX(I,J,K,IEL) = UX
                     VY(I,J,K,IEL) = UY
                     VZ(I,J,K,IEL) = UZ
                     VTRANS(I,J,K,IEL,IFIELD)=RHO
                     PHIG(I,J,K,IEL)=PHI
                     PR(I,J,K,IEL) =PRES
                     DO EQNUM=1,TOTEQ
                        U(I,J,K,EQNUM,IEL)=VARSIC(EQNUM)
                     ENDDO
                     IF (IFVISC) VDIFF(I,J,K,IEL,IFIELD)=MU
                  ELSE
                     T(I,J,K,IEL,IFIELD-1) = TEMP
                     IF (IFVISC) THEN
                        IF (IFIELD .EQ. 2)VDIFF(I,J,K,IEL,IFIELD)=UDIFF
                        IF (IFIELD .EQ. 3)VDIFF(I,J,K,IEL,IFIELD)=LAMBDA
                     ENDIF
                  ENDIF
