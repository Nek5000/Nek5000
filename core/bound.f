...
C=======================================================================
      ...
      do ie=2,nelv
         ...
         ioff = nxyz*(ie-1) + 1
         if(.not.if3d) call bnd_el2d(xm1(ioff,1,1,1)
     $      ,ym1(ioff,1,1,1),bnd_el(1,1,ie))
         if(     if3d) call bnd_el3d(xm1(ioff,1,1,1)
     $      ,ym1(ioff,1,1,1),zm1(ioff,1,1,1),bnd_el(1,1,ie))
         do d=1,ndim
c            print *, ie,bnd_el(1,d,ie),bnd_el(2,d,ie)
            bnd_all(1,d)=min(bnd_all(1,d),bnd_el(1,d,ie))
            bnd_all(2,d)=max(bnd_all(2,d),bnd_el(2,d,ie))
         enddo
c        xmx = max(xmx,glmax(zm1(ioff,1,1,1),nxyz))
c        write(6,*) ie,ioff,xmx,' xmx'
      enddo

c      do d=1,ndim
c         print *, 'tbound',d,bnd_all(1,d),bnd_all(2,d)
c      enddo
c     call exitt
