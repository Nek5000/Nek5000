...
!!$ACC& VECTOR_LENGTH(128) PRIVATE(wr,ws,wt)
      ...
      !$acc enter data copyin(wx,w3m2,x,nxyz2,ta1,rm2,dxtm12,nx1,nx2,ta2,nyz2)
      !$acc enter data copyin(iym12,ny2,ny1,nxy1,izm12,nz2,dtx,nz1,sm2)
      !$acc enter data copyin(ixtm12,dym12,nxyz1,tm2,dzm12)

      !$ACC PARALLEL LOOP COLLAPSE(2) GANG WORKER VECTOR

       ! nxyzt2 = nelv * nx2 * ny2 * nz2
       nxyzt2 = nxyz2 * nelv

!CCCCCCCCCCCCCC step 1 CCCCCCCCCCCCC
       ! change size of w3m2!!) 
       call col3_acc(wx,w3m2,x,nxzyt2)

       ! check siz of wx !?
       call col3_acc (ta1,wx,rm2e,nxyzt2)
...
!$ACC parallel loop collapse(4) 
       do e = 1,nelv
          ...
          tmp2 = 0.0                
!$ACC LOOP SEQ            
            do l=1,n    ! serial loop, no reduction needed
! Change ta2(nx2,ny2,nz2) to ta2(nx2,ny2,nz2,nelv)
               tmp1 = tmp1 + dxtm12(i,l)*ta1(l,j,k,e)
            enddo
            ta2(i,j,k,e) = tmp2  
         enddo
         enddo
...
!$ACC LOOP SEQ            
            do l=1,n    ! serial loop, no reduction needed
               tmp1 = tmp1 + iym12(i,l)*ta2(l,j,k,e)
            enddo
            ta1(i,j,k,e) = tmp1
         enddo
         enddo
...
! change the size of wx      
     call col3_acc (ta1,wx,sm2,nxyzt2)
...
!$ACC LOOP SEQ            
            do l=1,n    ! serial loop, no reduction needed
               dttmp = dttmp + iztm12(i,l)*ta1(l,j,k,e)
            enddo
            dtx(i,j,k,e) = dttmp
         enddo
         enddo
...
!CCCCCCCCCCCCCC step 2 CCCCCCCCCCCCC
...
!$ACC LOOP SEQ            
            do l=1,n    ! serial loop, no reduction needed
! Change ta2(nx2,ny2,nz2) to ta2(nx2,ny2,nz2,nelv)
               tmp1 = tmp1 + ixtm12(i,l)*ta1(l,j,k,e)
            enddo
            ta2(i,j,k,e) = tmp2  
         enddo
         enddo
...
!$ACC LOOP SEQ            
            do l=1,n    ! serial loop, no reduction needed
               tmp1 = tmp1 + dym12(i,l)*ta2(l,j,k,e)
            enddo
            ta1(i,j,k,e) = tmp1
         enddo
         enddo
...
!CCCCCCCCCCCCCC step 3 CCCCCCCCCCCCC


!!! Change the size of wx     
    call col3_acc (ta1,wx,tm2,nxyzt2)    
...
!$ACC LOOP SEQ            
            do l=1,n    ! serial loop, no reduction needed
               dttmp = dttmp + dztm12(i,l)*ta1(l,j,k,e)
            enddo
            dtx(i,j,k,e) = dtx(i,j,k,e) + dttmp
         enddo
         enddo
...
!CCCCCCCCCCCC end CCCCCCCCCCCCCCCCCc     
C
#ifdef TIMER
      tcdtp=tcdtp+(dnekclock()-etime1)
#endif

      !$ACC UPDATE DEVICE(wx,w3m2,x,nxyz2,ta1,rm2,dxtm12,nx1,nx2,ta2,nyz2)
      !$ACC UPDATE DEVICE(iym12,ny2,ny1,nxy1,izm12,nz2,dtx,nz1,sm2)
      !$ACC UPDATE DEVICE(ixtm12,dym12,nxyz1,tm2,dzm12)
